...
void lenet(Context<word>* v1, const Encoder<word>& v2, const EvaluationKey<word>& v3, const EvkMap<word>& v4, 
                    const std::array<Ciphertext<word>, 1>& v5, std::array<Ciphertext<word>, 1>& v6);
void lenet__encrypt__arg0(Context<word>* v1, const Encoder<word>& v2, const EvaluationKey<word>& v3, float* v4,
                           UserInterface<word>* v5, std::array<Ciphertext<word>, 1>& v6);
void lenet__decrypt__result0(Context<word>* v1, const Encoder<word>& v2, const EvaluationKey<word>& v3,
...
void run_lenet(std::vector<float> &test_input, std::vector<float> &result, std::vector<float> &torch_output) {
  ...
  lenet__encrypt__arg0(ctx.get(), ctx->encoder_, evk, &test_input[0], ui.get(),
                       encrypted_input);
  // lenet on encrypted input
   std::array<Ciphertext<word>, 1> encrypted_result;
  lenet(ctx.get(), ctx->encoder_, evk, evk_map, encrypted_input, encrypted_result);
  // decrypt result
  lenet__decrypt__result0(ctx.get(), ctx->encoder_, evk, encrypted_result, ui.get(),
                          &result[0]);
...
TEST(LeNet, OutputMatchesTorch) {
  run_lenet(test_input, result, torch_output);
...
TEST(LeNet, OutputShape) {
  EXPECT_EQ(result.size(), 10);
  EXPECT_EQ(result.size(), torch_output.size());
}

TEST(LeNet, FiniteandNonZero) {
